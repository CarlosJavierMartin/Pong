...
void gameReady(){
  ...
  ball = new Ball(20, width/2, height/2);
  textSize(32);
  text("Presiona \"Enter\" para comenzar",5,40);
  text("Presiona \"Retroceso\" para una nueva partida",5,85);
  text("Marcador:  " + player1Score + " - " + player2Score,5,130);
  text("Jugador 1", 10, 350);
  text("Jugador 2", 845, 350);
  text("Controles:",5,600);
  text("Jugador 1: 'W' moverse hacia arriba, 'S' moverse hacia abajo", 10, 645);
  text("Jugador 2: 'Flechita arriba' moverse hacia arriba, 'Flechita abajo' moverse hacia abajo", 10, 690);
  text("moverse hacia abajo", 10, 735);
...
void draw(){
  if (keyPressed){
    if (key == ENTER && !bool){
      ...
      int spx = xsing * (int)random(3,5);
      int spy = ysing * (int)random(3,5);
...
void ballPlayerCollide(){
  if (ball.getX() <= padding1+15 && ball.getX() > padding1 && ball.getY() > player1.getPos() && ball.getY() <= player1.getPos()+80 && playerTurn == 1){
    if (notCorner(1)){
      ...
      if (ball.setSpeed(-ball.getxSpeed(), ball.getySpeed(), 0.25)){
        speedSound.play();
      }
    } else {
      if (getCorner(1) == 1){
      System.out.println("Jugador 1b+1 " + ball.getY() + " " +player1.getPos() + " " + ball.getX() + " " + padding1);
        if (ball.setSpeed(Math.abs(ball.getxSpeed()), -Math.abs(ball.getySpeed()), 0.25)){
          speedSound.play();
        }
      } else {
      System.out.println("Jugador 1b-1  " + ball.getY() + " " +player1.getPos() + " " + ball.getX() + " " + padding1);
